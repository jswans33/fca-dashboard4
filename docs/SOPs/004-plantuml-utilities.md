...
and view diagrams for the FCA Dashboard project. PlantUML is a tool that allows
...
2. No manual updates to the HTML viewer are required - it dynamically detects
   all available diagram folders and files.
...
6. **Make commands not working**
   - Solution: Ensure you're using the correct path to the Makefile with
     `-f fca_dashboard/utils/puml/Makefile`
