...
class PredictionPipeline(BasePipeline):
    ...
    def _initialize_stages(self) -> None:
        ...
        if self.config.get("return_probabilities", False):
            from nexusml.core.pipeline.stages.prediction import ProbabilityPredictionStage
        ...
        if self.config.get("output_path"):
            from nexusml.core.pipeline.stages.output import OutputSavingStage
            self.add_stage(self.container.resolve(OutputSavingStage))
        else:
            logger.warning("No output_path specified in config, results will not be saved")
