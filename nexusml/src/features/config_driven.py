...
from nexusml.src.features.base import BaseConfigDrivenFeatureEngineer
from nexusml.src.features.interfaces import FeatureTransformer
from nexusml.src.features.registry import (
    create_transformer,
...
class ConfigDrivenFeatureEngineer(BaseConfigDrivenFeatureEngineer):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if "description" in transformed_data.columns and not any(
            "normalized" in col for col in transformed_data.columns
        ):
            transformed_data["description_normalized"] = transformed_data["description"]

        if "service_life" in transformed_data.columns and not any(
            "scaled" in col for col in transformed_data.columns
        ):
    ...
    def create_transformers_from_config(self) -> List[FeatureTransformer]:
        ...
        if "column_mappings" in self.config:
            transformer = create_transformer(
                "column_mapper", mappings=self.config["column_mappings"]
            )
            transformers.append(transformer)
        ...
        if "classification_systems" in self.config:
            for system in self.config["classification_systems"]:
                ...
                kwargs = {
                    "source_column": system_copy.get("source_column")
                    or system_copy.get("source_columns", []),
                    "target_column": system_copy["target_column"],
