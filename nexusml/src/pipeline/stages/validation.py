...
from nexusml.src.data.validation.interfaces import (
    ColumnValidator,
    DataFrameValidator,
    DataValidator,
)
from nexusml.src.data.validation.validators import (
    BasicColumnValidator,
    BasicDataFrameValidator,
    ConfigDrivenValidator,
)
from nexusml.src.pipeline.context import PipelineContext
from nexusml.src.pipeline.stages.base import BaseValidationStage
...
class ConfigDrivenValidationStage(BaseValidationStage):
    ...
    def validate_data(self, data: pd.DataFrame, **kwargs) -> Dict[str, Any]:
        ...
        return {
            ...
            "issues": [
                str(result) for result in validation_report.results if not result.valid
            ],
        }
...
class ColumnValidationStage(BaseValidationStage):
    ...
    def validate_data(self, data: pd.DataFrame, **kwargs) -> Dict[str, Any]:
        ...
        critical_columns = kwargs.get(
            ...
            self.config.get(
                "critical_columns",
                ["equipment_tag", "category_name", "mcaa_system_category"],
            ),
        )
    ...
    def validate_data(self, data: pd.DataFrame, **kwargs) -> Dict[str, Any]:
        ...
        for col, expected_type in column_types.items():
            ...
            try:
                ...
                type_issues.append(
                    f"Column '{col}' cannot be converted to {expected_type}: {str(e)}"
                )
