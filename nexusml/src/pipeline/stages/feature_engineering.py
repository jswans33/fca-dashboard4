...
from nexusml.src.features.config_driven import ConfigDrivenFeatureEngineer
from nexusml.src.features.interfaces import FeatureEngineer
from nexusml.src.pipeline.context import PipelineContext
from nexusml.src.pipeline.stages.base import BaseFeatureEngineeringStage
    ...
    def engineer_features(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        for combo in text_combinations:
            ...
            if available_columns:
                ...
                result[name] = (
                    result[available_columns].fillna("").agg(separator.join, axis=1)
                )
            else:
        ...
        for config in numeric_configs:
            ...
            if name in result.columns:
                ...
                if dtype == "float":
                    ...
                    result[new_name] = (
                        pd.to_numeric(result[new_name], errors="coerce")
                        .fillna(fill_value)
                        .astype(int)
                    )
            else:
    ...
    def engineer_features(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        for hierarchy in hierarchies:
            ...
            if available_parents:
                ...
                result[new_col] = (
                    result[available_parents].fillna("").agg(separator.join, axis=1)
                )
            else:
    ...
    def engineer_features(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        if available_fields:
            result["combined_text"] = (
                result[available_fields].fillna("").agg(" ".join, axis=1)
            )
        else:
