...
from nexusml.src.pipeline.pipelines.evaluation import EvaluationPipeline
from nexusml.src.pipeline.pipelines.prediction import PredictionPipeline
from nexusml.src.pipeline.pipelines.training import TrainingPipeline
...
class PipelineFactory:
    ...
    def create_pipeline(
        self, pipeline_type: str, config: Optional[Dict[str, Any]] = None
    ) -> BasePipeline:
        """
    ...
    def register_pipeline_type(
        self, name: str, pipeline_class: Type[BasePipeline]
    ) -> None:
        """
    ...
    def create_data_loader(self, config: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            if data_loader_class is None:
                ...
                from nexusml.src.pipeline.interfaces import DataLoader
    ...
    def create_model_serializer(self, config: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            if model_serializer_class is None:
                # Try to resolve it from the container
                from nexusml.src.pipeline.interfaces import ModelSerializer
            ...
            raise PipelineFactoryError(
                f"Error creating model serializer: {str(e)}"
            ) from e
    ...
    def create_data_preprocessor(self, config: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            if data_preprocessor_class is None:
                # Try to resolve it from the container
                from nexusml.src.pipeline.interfaces import DataPreprocessor
            ...
            raise PipelineFactoryError(
                f"Error creating data preprocessor: {str(e)}"
            ) from e
    ...
    def create_feature_engineer(self, config: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            if feature_engineer_class is None:
                # Try to resolve it from the container
                from nexusml.src.pipeline.interfaces import FeatureEngineer
            ...
            raise PipelineFactoryError(
                f"Error creating feature engineer: {str(e)}"
            ) from e
    ...
    def create_model_builder(self, config: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            if model_builder_class is None:
                # Try to resolve it from the container
                from nexusml.src.models.base import ModelBuilder
    ...
    def create_model_trainer(self, config: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            if model_trainer_class is None:
                # Try to resolve it from the container
                from nexusml.src.models.training.base import ModelTrainer
    ...
    def create_model_evaluator(self, config: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            model_evaluator_class = self.registry.get(
                "model_evaluator", "classification"
            )
            if model_evaluator_class is None:
                # Try to resolve it from the container
                from nexusml.src.models.base import ModelEvaluator
            ...
            raise PipelineFactoryError(
                f"Error creating model evaluator: {str(e)}"
            ) from e
    ...
    def create_predictor(self, config: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            if predictor_class is None:
                # Try to resolve it from the container
                from nexusml.src.pipeline.interfaces import Predictor
    ...
    def create_pipeline_from_config(self, config_path: str) -> BasePipeline:
        ...
        try:
            ...
            if not pipeline_type:
                raise PipelineFactoryError(
                    "Pipeline type not specified in configuration"
                )
            ...
            if isinstance(e, PipelineFactoryError):
                raise
            raise PipelineFactoryError(
                f"Error creating pipeline from config: {str(e)}"
            ) from e
